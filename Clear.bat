...
echo sLinkFile = "Z:\Install\ProgramData\Microsoft\Windows\Start Menu\Programs\System Tools\Включить поддержку Appx.lnk" >> %VBS%
...
set VBS=Z:\TEMP.vbs
echo Set oWS = WScript.CreateObject("WScript.Shell") >> %VBS%
echo sLinkFile = "Z:\Install\ProgramData\Microsoft\Windows\Start Menu\Programs\System Tools\Выключить поддержку Appx.lnk" >> %VBS%
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> %VBS%
...
set VBS=Z:\TEMP.vbs
echo Set oWS = WScript.CreateObject("WScript.Shell") >> %VBS%
echo sLinkFile = "Z:\Install\ProgramData\Microsoft\Windows\Start Menu\Programs\Accessories\Калькулятор.lnk" >> %VBS%
echo Set oLink = oWS.CreateShortcut(sLinkFile) >> %VBS%
...
for /f "tokens=*" %%i in ('dir Z:\Install\Windows\WinSxS\Backup /b /a:-d') do (
	takeown /f "Z:\Install\Windows\WinSxS\Backup\%%~i"
